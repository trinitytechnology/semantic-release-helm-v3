...
export default async (pluginConfig, { nextRelease: { type, version, notes }, logger, env }) => {
  ...
  if (pluginConfig.onlyUpdateVersion) {
    doc.set("version", nextVersion);
    logger.log("Updating Chart.yaml with version %s.", nextVersion);
  } else {
    doc.set("version", nextVersion);
    doc.set("appVersion", nextAppVersion);
    logger.log("Updating Chart.yaml with version %s and appVersion %s.", nextVersion, nextVersion);
  }
