...
import { HELM_MINIMUM_VERSION } from "./constant";
...
function validateChartPath(pluginConfig, env, errors) {
  if (!pluginConfig.chartPath && !env.CHART_PATH) {
    errors.push(
      "Missing argument: chartPath, set by plugin config chartPath key or CHART_PATH in environment"
    );
  }
}
...
async function handleChartMuseumRegistry(registryHost, env, errors) {
  if (!/^https?/i.test(registryHost)) {
    errors.push(
      "Invalid registry. For ChartMuseum, a repository URL is required."
    );
    return;
  }
...
async function verifyRegistryLogin(host, username, password) {
  await execa(
    "helm",
    ["registry", "login", "--username", username, "--password-stdin", host],
    {
      input: password,
...
async function addChartRepository(url, username, password) {
  const args = ["repo", "add", "chartmuseum", url];
